...
from os import chdir, environ, getcwd, mkdir, remove, access, W_OK, pardir
...
def doExecute(cmnd, dir, timeout, line_num):
    # Debug
    # print("Debug::doExecute()::calling exeCpp() at the begining")
    # exeCpp()
    ...
    try:
        ...
        full_cmnd = "{} {}; {} {}".format("cp ../gitlet", getcwd(), CPP_COMMAND, cmnd)
        #Debug
        #print("Debug::doExecute()::full_cmnd: ", full_cmnd)
        ...
        if DEBUG:
            ...
            if next_cmd == "s":
                ...
                full_cmnd = "{} {}".format(CPP_COMMAND, cmnd)
                timeout, skip_first_line = None, True

        # Debug
        #print("Debug::doExecute()::CWD = ", getcwd())
        
        # Debug
        # print("Debug::doExecute()::calling exeCpp()")
        # out = exeCpp()

        out = doCommand(full_cmnd, timeout, skip_first_line)
        return "OK", out
    except CalledProcessError as excp:
        return ("java gitlet.Main exited with code {}".format(excp.args[0]), excp.output)
...
def doCommand(full_cmnd, timeout, skip_first_line=False):
    # Debug
    #print("Debug::doCommand()::full_cmnd: ", full_cmnd)

    # Test
    # out = check_output(full_cmnd, shell=True, timeout=timeout)

    out = check_output(full_cmnd, shell=True, universal_newlines=True,
                        stdin=DEVNULL, stderr=STDOUT, timeout=timeout)

    #Debug
    #print("Debug::doCommand()::out: ", out)
    ...
    try:
        ...
        while True:
            ...
            if Match(r'\s*#', line) or Match(r'\s+$', line):
                ...
                # Debug
                # print("Debug::doTest()::calling exeCpp()")
                # exeCpp()
                #Debug
                #print("Debug::doTest():b2")
                #print("Debug::doTest()::cmnd: ", cmnd)

                msg, out = doExecute(cmnd, cdir, timeout, line_num)
                
                #Debug
                #print("Debug::doTest():b3")
                #print("Debug::doTest()::msg:  ", msg)
                #print("Debug::doTest()::out:  ", out)
                if verbose:
                    if out:
...
def exeCpp():
    ...
    os.close(temp)
    s = subprocess.check_output(" ./gitlet init", shell=True, stdin=data)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prog_dir is None:
            ...
            # prog_parent_dir = abspath(join(prog_dir, pardir))

            # Debug:
            # print(prog_dir)
            # print(prog_parent_dir)
            # print(join(prog_parent_dir, 'gitlet'))
    ...
    # Debug: execute a cpp program by calling check_output()
    # print("Debug::main()::calling exeCpp()")
    # out = exeCpp()


    for test in files:
        # Debug:
        #print("Debug::Main()::test: ", test)
        try:
            if not exists(test):
                num_tests -= 1
